...
import { runThroughUpgradeScripts, runConfigThroughUpgradeScripts } from '../internal/upgrade.js'
...
export abstract class InstanceBase<TConfig> implements InstanceBaseShared<TConfig> {
	...
	private async _handleInit(msg: InitMessage): Promise<InitResponseMessage> {
		return this.#lifecycleQueue.add(async () => {
			 ...
			 */
			const { updatedConfig } = runConfigThroughUpgradeScripts(
	...
	private async _handleUpgradeActionsAndFeedbacks(
		...
		return runThroughUpgradeScripts(msg.actions, msg.feedbacks, this.#upgradeScripts, this.#lastConfig)
	}
